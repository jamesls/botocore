...
class TestProcessProvider(BaseEnvVar):
    ...
    def test_can_retrieve_via_process(self):
        ...
        self.invoked_process.communicate.return_value = (json.dumps({
            'Version': 1,
            'AccessKeyId': 'foo',
            'SecretAccessKey': 'bar',
            'SessionToken': 'baz',
    ...
    def test_non_zero_rc_raises_exception(self):
        ...
        self.invoked_process.communicate.return_value = ('', 'Error Message')
        self.invoked_process.returncode = 1

        provider = self.create_process_provider()
        exception = botocore.exceptions.CredentialRetrievalError
        with self.assertRaisesRegexp(exception, 'Error Message') as e:
            creds = provider.load()
